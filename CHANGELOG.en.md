...
- **Ruled out one alternative explanation.** A comparison of normalized features showed that the imitation drafts had a higher density of named entities than the originals (9.2 vs 4.7), so "the judges just thought the drafts were too short" does not hold; the only things genuinely separable by mechanical means were connectives and emotion words.
...
- **A local web tool** (Python standard library + vanilla frontend, zero dependencies, fully offline):
  ...
  - hints for a whole book prepared in a background batch, with a token estimate available up front (clearly labeled as an estimate)
...
- **Whitespace normalization**: articles copied from web pages carry leading and trailing spaces and whole blank lines, which made the split produce empty fragments and shifted the sentence numbering. Import now normalizes whitespace only and **does not move a single content character**.
